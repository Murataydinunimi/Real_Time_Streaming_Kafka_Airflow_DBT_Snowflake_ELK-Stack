...
FROM TAXI_APP.TRANSACTIONS_RAW.TAXI_CALLS_JSON)


SELECT * from json_calls_table
WHERE DT_INGESTION_TIMESTAMP is not null

  AND DT_INGESTION_TIMESTAMP > (select max(DT_INGESTION_TIMESTAMP) from TAXI_APP.TRANSACTIONS.taxi_calls_table)
